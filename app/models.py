...
class SMBServer(models.Model):
    """
    Модель для хранения информации о серверах Samba.
    """
...
class MnemonicScheme(models.Model):
    ...
    """
    image = models.ImageField(upload_to='mnemoschemes/', blank=False, verbose_name='Мнемосхема')
    slug = models.SlugField(max_length=30, blank=False, unique=True, db_index=True, verbose_name='Слаг')
    navmenu_name = models.CharField(max_length=100, blank=False, verbose_name='Название для меню')
...
class HistoricalValue(models.Model):
    ...
    sensor = models.ForeignKey('Sensor', on_delete=models.DO_NOTHING, related_name='historic_values', null=True, verbose_name='Датчик')
